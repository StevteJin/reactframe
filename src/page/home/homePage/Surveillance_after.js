...
/!*                for (let i = 0; i < arr.length; i++) {
    ...
    faceDatabase(){
        $(".faceDatabase").show();
        axios.post(global.Url+'business/selectFacebank',{name:'',cardid:''}).then((res) => {
            const result = res.data.data;
            if(result) {
                console.log(result);
                setTimeout(function(){
    ...
    RenIdSearch(item){
        ...
        if(item===1){
            ...
            data={name:RenName,cardid:RenID,}
        }else{
            data={name:'',cardid:''}
        }
    ...
    renTab(type){
        if(type===1){
            Surveillance.this.setState({renTab:true})
        }else{
            Surveillance.this.setState({renTab:false})
        }
    }
    render(){
            ...
            <Fragment>
                ...
                <div className="RenBut">
                    ...
                    <div className="faceDatabase">
                        <h1>
                            <span style={renStyle}>人脸库</span><i onClick={()=>this.RenClose(2)}></i>
                        </h1>
                            ...
                            <div className="ContList">
                                {
                                    RenList.length > 0 && RenList.map((ele,index)=>{
                                        return(
                                            <li key={index} onClick={()=>this.onTable(ele.imgurl)}>
                                                <img src={ele.imgurl} alt=""/>
                                                <p><span>姓名：</span>{ele.name}</p>
                                                <p><span>性别：</span>{ele.sex=== -1 ?'女':'男'}</p>
                                                <p><span>证件号：</span></p>
                                                <p>{ele.cardid}</p>
                                            </li>
                                        )
