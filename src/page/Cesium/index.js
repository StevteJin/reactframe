        ...
        window.receiveMessageFromIndex = function ( e ) {
            if(e !== undefined){
                switch(e.data.switchName){
                    case 'MessageBox':
                        console.log(e.data);
                        $(".PageShowOne").show().find("h1").html(e.data.text.Title + e.data.text.txt);
                        //global.Time();
                        break;
                    default:
                        return null;
        ...
        axios.get(global.Url+'map/build/list',{}).then((res) => {
            ...
            if(result && res.data.msg === 'success') {
                for (let i = 0; i < result.length; i++) {
                    axios.post(global.Url+'map/floor/list',{build_id:result[i].id}).then((res) => {
