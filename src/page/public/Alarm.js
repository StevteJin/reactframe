...
class Alarm extends Component{
                   ...
                   return
                    ...
                    switch (WebListLength) {
                        case 0:
                            ...
                            $("#NumList"+ e.data.category_id).css({'display':'block'});
                            $("#NumOneList"+ e.data.category_id).css({'display':'block'});
                            $(".OneAlarm").find("span").css({'display':'block'});
                            Alarm.this.setState({
                                info:global.AlarmNumber,
                                listData:e.data,
                                TatolNum: Alarm.this.state.TatolNum + 1
                            })
                            WebList.push(JSON.parse(e.data));
                            //Common.AlarmBox(JSON.parse(e.data));
                            Common.AlarmBox(JSON.parse(e.data));
                            //Common.ModelClickEvent(e.data.device_info.device_code);   //视频自动播放
                            let data={
                        ...
                        default:
                            ...
                            for(var i=0;i<Olist.length;i++){
                                if((JSON.parse(e.data).event_info.device_code === Olist[i].event_info.device_code) && (JSON.parse(e.data).event_info.start_time === Olist[i].event_info.start_time)){
                                    console.log(JSON.parse(e.data).event_info.device_code,Olist[i].event_info.device_code,'相同');
                                    console.log(JSON.parse(e.data).event_info.start_time,Olist[i].event_info.start_time,'相同');
                                    break;
                                    return null;
                                }else{
                                    console.log('不同');
                    ...
                    switch(e.data.switchName){
                        case 'removeBubbleBombbox':
                            ...
                            if(parseInt($("#NumList"+ e.data.category_id).html()) === 0 || parseInt($("#NumList"+ e.data.category_id).html()) < 0){
                                $("#NumList"+ e.data.category_id).css({'display':'none'});
                                $("#NumOneList"+ e.data.category_id).css({'display':'none'});
                                $(".OneAlarm").find("span").css({'display':'none'});
                            }else{
                                $(".OneAlarm").find("span").css({'display':'block'});
    ...
    onTable(id){
        ...
        time=time+' 00:00:00';
        //console.log(id,time);
        axios.post(global.Url+'event/info/list',{device_category:id,start_time:time}).then((res) => {
            const results = res.data.data;
            if(results && res.data.msg === 'success') {
                //console.log(results);
                this.setState({
    ...
    GMTToStr(time){
     ...
     render(){
        const {device,detailsList,TatolNum,info} = this.state;
         return(
             <Fragment>
                 <div className="AlarmButton OneAlarm" onClick={()=>this.onAlarm()}>今日报警 <span>{info}</span></div>
